...
TEST(BenchmarkInputStreamTest, BenchmarkZeroCopyInputStreamSimple) {
  const std::string json_msg_input[] =
      {R"({"Hello":"World!"})",
       R"([{"Hello":"World!"}])",
       R"([{"Hello":"World!"},{"Hello":"World, Again!"}])"};

  for (const std::string& json_msg: json_msg_input) {
    BenchmarkZeroCopyInputStream is(json_msg, 1);
